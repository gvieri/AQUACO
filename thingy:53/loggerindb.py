...
def getOptions(args=sys.argv[1:]):
    ...
    parser.add_argument('-f','--filename',help='output filename. ', action='store', default='thingy53log.txt' )
    parser.add_argument('-s','--sleeptime',help='sleep time between readings ', action='store',default=600 )
    parser.add_argument('-db','--database',help='sqlite3 database filename  ', action='store', default='sampleaquaco.sqlite' )
...
#    print ("\n", "validate :", validateJSON(completejson))
    ...
    if opt.repeat:
        ...
        if counter==0: break 
    #### save on db 
    data=completedict['payload']['values']
    T_av=data[0][0]
    P_av=data[0][1]
    H_av=data[0][2]
    G_av=data[0][3]
    if opt.debug:
        ...
        print (H_av)
        print (G_av)
    record=insert(Sample).values(orig_sample=json.dumps(completedict),T_av=T_av,P_av=P_av,H_av=H_av,G_av=G_av)
 #   print(record)
